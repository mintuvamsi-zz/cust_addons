...
class WebsiteSupportTicket(models.Model):
    ...
    mngr_email = fields.Char(string="Manager Email")
    # department_id = fields.Many2one('hr.employee',string="Department")
    desk_number = fields.Char(string="Desk Number")
    ...
    estimated_hours = fields.Char(string="Estimtaed Hours")
    ...
    assigned_line_item = fields.One2many('assigned.ticket.history.lines','website_id',string="Assigning Ticket Status History Lines")
    # pdb.set_trace()
    ...
    #     pdb.set_trace()
    #     emp_list = self.env['hr.employee'].search([('user_id','=',self.env.uid)])
    #     for emp in emp_list:
    #         result = emp.parent_id
    #         print("*************Manager is:",result)
    ...
    def _onchange_partner_id(self):
        if self.partner_id:
            ...
            # pdb.set_trace()
            usr= self.env['res.users'].search([('partner_id','=',self.partner_id.id)])
            # for emp in emp_list:
            #     result = emp.parent_id
            #     print("*************Manager is:",result)
            # self.mngr_email=emp.parent_id.user_id.partner_id.email
            emp=self.env['hr.employee'].search([('user_id','=',usr.id)])
            self.manager=emp.parent_id.user_id.partner_id.id
            # print("******************")
            # self._cr.EXECUTE
    ...
    #         pdb.set_trace()
    #         emp=self.env['hr.employee'].search([('user_id','=',usr.id)])
    #         self.manager=emp.parent_id.user_id.id
    #         self.mngr_email=emp.parent_id.user_id.email
    #         print("******************")
    ...
    #         'view_mode': 'form',
    #         'res_model': 'website.support.ticket.compose',
    #         'context': {'default_ticket_id': self.id, 'default_email': self.email, 'default_mngremail': self.mngr_email ,'default_subject': self.subject, 'default_approval': True, 'default_body': request_message},
    #         'target': 'new'
    #     }
    ...
    def create(self, vals):
        ...
        # pdb.set_trace()
        if new_id:
            usr= self.env['res.users'].search([('partner_id','=',new_id.partner_id.id)])
            emp=self.env['hr.employee'].search([('user_id','=',usr.id)])
            new_id.manager=emp.parent_id.user_id.partner_id.id
...
class assigned_ticket_history_lines(models.Model):
    _name = "assigned.ticket.history.lines"

    # pdb.set_trace()
    
    # assign_to = fields.Many2one(related='website_id.user_id', string="Assigned To", store=True)
    user_id = fields.Many2one('res.users', string="Assigned User")
    ...
    website_id = fields.Many2one('website.support.ticket',string="Website Reference")

assigned_ticket_history_lines()
